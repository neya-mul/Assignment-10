  ...
  if (!user?.id) {
    return (
      <div className="py-20 text-center text-white/40">
        <p className="text-lg font-semibold">Please log in to view your classes.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 text-white px-4 sm:px-6 lg:px-0">
      ...
      <div className="bg-[#0e0b1f]/60 backdrop-blur-xl border border-purple-500/10 rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-purple-500/5">
          ...
          {/* 🔒 সেফটি চেক: ডাটা কেবল অ্যারে হলেই ম্যাপ হবে, নাহলে এরর মেসেজ দেখাবে */}
          {Array.isArray(myClasses) ? (
            myClasses.map((c) => (
              <div
                key={c._id}
                className="p-4 sm:px-6 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 flex flex-col gap-3 hover:bg-purple-500/5 transition"
              >
                {/* CLASS INFO */}
                <div>
                  <h3 className="font-semibold text-white text-sm sm:text-base">{c.className}</h3>
                  <p className="text-xs text-white/40">{c.trainerName}</p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 sm:justify-center">
                  <TrainerClassUpdateModal c={c} />
                  <TrainerDeleteModal c={c} />
                  <button className="flex-1 sm:flex-none px-3 py-1.5 text-[11px] font-bold uppercase rounded-md bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 hover:bg-cyan-500/20 transition">
                    Students
                  </button>
                </div>
              </div>
          ...
          {Array.isArray(myClasses) && myClasses.length === 0 && (
            <div className="py-16 sm:py-20 text-center text-white/40 px-4">
              ...
              <p className="text-xs sm:text-sm mt-2">Your created classes will appear here.</p>
            </div>
          )}
